def calcular_score(indicadores):
    ...
    if pl is not None and 5 <= pl <= 15:
        score += 15
        pontos["P/L"] = 15
        detalhes.append("✅ P/L entre 5 e 15 (+15)")
    else:
        pontos["P/L"] = 0
    ...
    if roe is not None:
        ...
        if roe >= 20:
            score += 20
            pontos["ROE"] = 20
            detalhes.append("✅ ROE >= 20% (+20)")

        elif roe >= 15:
            score += 15
            pontos["ROE"] = 15
            detalhes.append("✅ ROE >= 15% (+15)")

        elif roe >= 10:
            score += 10
            pontos["ROE"] = 10
            detalhes.append("✅ ROE >= 10% (+10)")
    ...
    if dy is not None:
        ...
        if dy >= 8:
            score += 20
            pontos["Dividend Yield"] = 20
            detalhes.append("✅ Dividend Yield >= 8% (+20)")

        elif dy >= 6:
            score += 15
            pontos["Dividend Yield"] = 15
            detalhes.append("✅ Dividend Yield >= 6% (+15)")

        elif dy >= 4:
            score += 10
            pontos["Dividend Yield"] = 10
            detalhes.append("✅ Dividend Yield >= 4% (+10)")
